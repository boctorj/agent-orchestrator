...
6. **Rebase against latest main:**
   ...
   **On conflict:** try to resolve it. Keep your changes intact:
   ```sh
...
7. **Pre-commit self-review** — before staging, review your own diff:

   ```sh
   git diff      # the diff you're about to commit
   ```
   ...
   **(b) Arch-drift.** For each file you touched:
   ...
   Surface findings as fixes in your own diff before pushing. Findings
   that are real but outside this unit's scope go in the PR body's
...
10. Output the PR URL on its own line at the very end of your response, prefixed with `PR_URL:` exactly. The orchestrator parses this. Example final line:
   ```
