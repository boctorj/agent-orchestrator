...
def check_unit_pr(unit_id: str) -> str:
    ...
    **Deprecated** (F-014-U-2). The canonical surface is
    ``inspect_unit_health(unit_id, dry_run=True)`` — same read, plus
    full conflict / review / CI-drift coverage and a shadow-decision
    audit log. ``check_unit_pr`` remains as a thin alias that returns
    the legacy JSON shape (no ``applied_actions`` field) so existing
    ...
    ``reconcile_unit_pr(unit_id)`` (also deprecated) or the canonical
    ``inspect_unit_health(unit_id)``.
    """
    warnings.warn(
        "check_unit_pr is deprecated; use inspect_unit_health(unit_id, dry_run=True) instead.",
        DeprecationWarning,
        stacklevel=2,
    )
...
def reconcile_unit_pr(unit_id: str) -> str:
    ...
    **Deprecated** (F-014-U-2). The canonical surface is
    ``inspect_unit_health(unit_id, dry_run=False)`` — same merge
    transitions, plus event-only ``pr_conflict_detected`` /
    ``required_check_missing`` / ``ci_drift_detected`` signals and a
    shadow-decision audit log. ``reconcile_unit_pr`` remains as a thin
    alias preserving the legacy JSON shape (``reconciled`` + ``action``
    slug) and the cycle-log writer side effect on merged polls.
    ...
    """
    warnings.warn(
        "reconcile_unit_pr is deprecated; use inspect_unit_health(unit_id) instead.",
        DeprecationWarning,
        stacklevel=2,
    )
